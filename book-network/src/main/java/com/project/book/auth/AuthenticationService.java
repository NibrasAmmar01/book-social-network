...
public class AuthenticationService {
    ...
    private String generateActivationCode(int length) {
        ...
        for (int i = 0; i < length; i++){
            int randowIndex = secureRandom.nextInt(characters.length());
            codeBuilder.append(characters.charAt(randowIndex));
        }
    ...
    public void activateAccount(String token) throws MessagingException {
        ...
        if(LocalDateTime.now().isAfter(savedToken.getExpiresAt())){
            ...
            throw new RuntimeException("Activation Token has expired. A new Token has been sent to the same email addresse");
        }
